...
Pour transition : la phrase d'ACCROCHE ORALE dite juste AVANT le résumé (2-6 mots + virgule), qui rebondit sur le CONTENU réel — jamais "On commence"/"On continue" répété mécaniquement.

RÈGLE DE COHÉRENCE : compare le segment au précédent avant d'écrire la transition (utilise le champ "bloc" que tu renseignes toi-même : "resultats", "affiches" ou "stat"). N'utilise "Changement de registre,"/"Direction..."/"cette fois" QUE si on passe vraiment d'un bloc à l'autre (résultats → affiches, ou pays différent). Si deux segments consécutifs restent dans le même bloc (ex: deux résultats à la suite), utilise un connecteur additif ("Autre résultat de la journée,", "Toujours dans les résultats,") — jamais un mot qui prétend à tort qu'on change de sujet.
...
def _demo_worldcup(n: int) -> dict:
    topics = [
        ("France 2 à 1 Brésil : qualification arrachée", "resultats",
        ...
        ("Espagne-Argentine, choc des demi-finales", "affiches",
        ...
        ("Le Maroc continue de surprendre", "affiches",
         "Toujours du côté des affiches à venir,",
        ...
        ("La finale se jouera à New York", "stat",
         "Autre info marquante avant la finale,",
    ...
    news = [{"titre": t[0], "bloc": t[1], "transition": t[2], "resume": t[3], "source": t[4],
